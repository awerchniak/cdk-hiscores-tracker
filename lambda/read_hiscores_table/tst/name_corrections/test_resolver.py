...
def test_multi_hop_chain_from_middle_name() -> None:
    canonical, aliases = resolve_aliases("sstate")
    assert canonical == "state 0"
    assert aliases == {"State Lad", "sstate", "state 0"}


def test_multi_hop_chain_from_canonical_name() -> None:
    canonical, aliases = resolve_aliases("state 0")
    assert canonical == "state 0"
    assert aliases == {"State Lad", "sstate", "state 0"}


def test_independent_rename_pairs_do_not_cross_contaminate() -> None:
    _, aliases = resolve_aliases("GI Pliny")
    assert "GI Jobra" not in aliases
    assert "DrPlingo0" not in aliases


def test_synthetic_two_hop_chain_with_injected_map() -> None:
    name_map = {"A": "B", "B": "C"}
...
def test_canonical_helper_directly() -> None:
    name_map = {"X": "Y"}
