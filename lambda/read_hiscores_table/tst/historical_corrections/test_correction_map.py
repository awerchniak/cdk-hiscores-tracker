...
def test_deployed_and_correct_differ():
    for era in CORRECTION_ERAS:
        assert (
...
def test_correct_has_more_or_different_activities():
    for era in CORRECTION_ERAS:
...
def test_era_0_baseline_count():
    assert len(ERA_0) == 59


def test_era_1_nex_added():
    assert len(ERA_1) == 60
    assert "Nex" in ERA_1
    assert "Nex" not in ERA_0


def test_era_20_final_count():
    assert len(ERA_20) == 89


def test_nex_era_renames():
...
def test_correction_era_cached_property():
    era = CorrectionEra(
