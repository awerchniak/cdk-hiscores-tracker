...
def _sum_nested(a, b):
    ...
    for key, a_val in a.items():
        ...
        result[key] = (
            _sum_nested(a_val, b_val) if isinstance(a_val, dict) else a_val + b_val
        )
    return result


def merge_period_collisions(items):
            ...
            try:
                for field in ("skills", "activities"):
                    if field in merged and field in other:
                        merged[field] = _sum_nested(merged[field], other[field])
                merged["divisor"] += other["divisor"]
